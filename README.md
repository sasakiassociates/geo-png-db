...
This is the specification file for geo-png-db
...
No server queries = Fast - fluid updates. If rendered on GPU.

#### Novel Queries
Keeping the information as raw data allows for combining massive datasets on the fly in a web browser -- vs just representing the data as graphic image - or as raw aerials…
...
Because each tile is represented independently as a 256x256 image, very lightweight processing can be used when generating tiles, and tiles can be processed in a massively parallel fashion using cloud-based architectures. This allows truly massive datasets to be processed in manageable chunks on affordable microservices (such as AWS Lambda or Google Cloud Functions).
